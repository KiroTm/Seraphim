...
import { Message, Collection, Role, EmbedBuilder, ClientEvents } from "discord.js";
...
export default class automod extends Event {
    ...
    for (const rules of automodData.rules.values()) {
      ...
      switch (rules.type) {
        case automodtype.BannedWords: {
          ...
          const slursFound = this.checkForSlurs(content, rules);
          if (slursFound) this.handleViolation(message, rules)
          break;
        }
        ...
        case automodtype.MassMention: {
          ...
          if (mentionCount >= rules.config[0]?.Query! ?? 3) this.handleViolation(message, rules, advancedSettings)
          break;
        }

        case automodtype.ServerInvites: {
          const inviteRegex = /\b(?:discord\.com\/invite|discord\.gg)\/[a-zA-Z0-9]+/g;
          if (inviteRegex.test(content)) this.handleViolation(message, rules, advancedSettings)
          break;
        }
        ...
        case automodtype.MassEmoji: {
          ...
          if (emojiCount >= (rules.config[0]?.Query ?? 3)) this.handleViolation(message, rules, advancedSettings)
          break;
        }
        ...
        case automodtype.LinkCooldown: {
          ...
          if (currentTime - userLastLinkTimestamp < cooldownLimit) {
            this.handleViolation(message, rules, advancedSettings);
          } else {
        ...
        case automodtype.NewLines: {
          if (/\n\s*\n\s*/.test(content)) this.handleViolation(message, rules, advancedSettings)
          break;
        }

        case automodtype.ChatFlood: {
          if (/(\w+)\1{50,}/.test(content)) this.handleViolation(message, rules, advancedSettings)
          break;
        }
        ...
        case automodtype.FastMessage: {
          ...
          recentTimestamps.length >= numMessagesAllowed ? this.handleViolation(message, rules, advancedSettings) : userTimestamps.set(author.id, [...recentTimestamps, currentTime]);
        ...
        case automodtype.AllCaps: {
          ...
          if (uppercasePercentage >= 0.75) this.handleViolation(message, rules, advancedSettings);
          break;
        }
      }
    };
  ...
  private handleViolation(message: Message, rules: AutomodSetupInterface, advancedSetting?: AdvancedSettingFields) {
    ...
    if (advanced) {
      ...
      if (advancedSetting?.Action) this.performCustomAction(rules, message, advancedSetting ?? undefined)
    }
    message.delete().catch(() => { });
    new ResponseClass().sendTemporaryMessage(message, {
      content: rules.customResponse ?? this.getResponse(rules.type, message),
      allowedMentions: { users: [message.author.id] }
    }, '3s', 'Create');
  }

  private getResponse(ruleType: automodtype, message: Message) {
  ...
  private checkForSlurs(content: string, slursConfig: AutomodSetupInterface) {
  ...
  private performCustomAction(rule: AutomodSetupInterface, message: Message, advancedSettings?: AdvancedSettingFields) {
    const { type } = rule
