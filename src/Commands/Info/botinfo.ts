	...
	callback: async ({ message, instance, client }: Callback) => {
		try {
			...
			const embedFields = [
				...
				{
					name: titles.stats,
					value: ` Users: \`${stats.users}\`\n Servers: \`${stats.guilds}\`\n Channels: \`${stats.channels}\``,
					inline: true,
				},
				{
					name: titles.misc,
					value: ` Lines of code: \`${codeStats.linesOfCode}\`\n Files: \`${codeStats.numOfFiles}\``,
					inline: true,
				},
				{
					name: titles.uptime,
					value: ` Client: ${uptimeStats.client}`,
					inline: true,
				},
				{
					name: titles.serverUsage,
					value: ` Heap: \`${usageStats.ramUsed}MB\` (Total: \`${usageStats.ramTotal}MB\`)\n CPU Load: ${usageStats.cpuLoad}`,
					inline: true,
				},
			];
...
function generalStatistics(instance: any) {
	...
	return {
		...
		users: client.guilds.cache
			.reduce((acc: any, val: any) => acc + (val.memberCount ?? 0), 0)
			.toLocaleString(),
	};
}
