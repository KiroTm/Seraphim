import { ApplicationCommandOptionType, Client, Embed, EmbedBuilder, Guild, GuildMember, User, UserResolvable } from "discord.js"
    ...
    callback: async ({ interaction, message, args, guild, client }: Callback) => {
        ...
        if (typeof target == 'object' && ('username' in target && 'globalName' in target)) {
            const replyObject = {
                embeds: [
                    new EmbedBuilder()
                    .setTitle(`${target.username}'s user avatar`)
                    .setColor('Blurple')
                    .setAuthor({
        ...
        } else if (typeof target == 'object' && ('roles' in target && 'guild' in target)) {
            const replyObject = {
                embeds: [
                    new EmbedBuilder()
