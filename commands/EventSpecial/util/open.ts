...
export default {
    ...
    callback: async ({ message, member, args, instance, channel, client }: Callback) => {
        ...
        const response = await channel.send({ content: `${message.author} Are you sure you want to open **${crateName}** \`x${amount}\``, embeds: [new EmbedBuilder().setTitle(`${crateName}`).setColor('Blue').setDescription(`${crateClass.Crates.find((_, key) => key.toLowerCase().includes(crateName.toLowerCase()))!.map((value, key) => `${value.name}`)}`).setFields({name: `You have:`, value: `**${crate.amount ?? 0}**`, inline: false}).setThumbnail(dropTypes[crate.name as CrateType].image).setFooter({ text: "This message will expire in 15 seconds.", iconURL: `${message.client.user.displayAvatarURL()}` })], components: [row] });
