...
export default {
    ...
    callback: async ({ args, message, interaction, guild, client }: Callback) => {
        ...
        const target: GuildMember | User | undefined = message ? (memberClass.fetch(guild, args.join(' '), message) as GuildMember ?? (await userClass.fetch(client, args.join(' '), message) as User || undefined)) : (interaction.options.getMember('user') as GuildMember ?? interaction.options.getUser('user') as User ?? interaction.member as GuildMember)
